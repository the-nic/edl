...
Usage:
    ...
    edl [--debugmode] [--loader=filename] [--memory=memtype] [--portname=portname] [--serial]
...
Description:
    printsahara                 # only print sahara info (if available) and exit
...
class main(metaclass=LogBase):
    ...
    def run(self):
        ...
        if mode == "sahara":
            sahara_cmd = conninfo["cmd"]
            if sahara_cmd != cmd_t.SAHARA_HELLO_REQ:
                self.sahara.cmd_reset_state_machine()
                conninfo = self.sahara.connect()
                sahara_cmd = conninfo["cmd"]
            if sahara_cmd == cmd_t.SAHARA_HELLO_REQ:
                if "data" in conninfo:
                    data = conninfo["data"]
                    ...
                    if data.mode == sahara_mode_t.SAHARA_MODE_MEMORY_DEBUG:
                        ...
                        sahara_info = self.sahara.cmd_info(version=version, skip_loader=(cmd == "printsahara"))
                        if sahara_info:
                            resp = self.sahara.connect()
                            mode = resp["mode"]
                            if "data" in resp:
